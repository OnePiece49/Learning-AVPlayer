...
class RangeSliderThumbLayer: CALayer {
    ...
    override func layoutSublayers() {
        ...
        switch thumbPosition {
            case .left:
                image = rangeSlider?.leftImage?.cgImage
                imageLayer.frame = CGRect(x: 0, y: 0, width: bounds.width * 3/4, height: bounds.height)
            case .right:
                image = rangeSlider?.rightImage?.cgImage
                imageLayer.frame = CGRect(x: bounds.width * 1/4, y: 0, width: bounds.width * 3/4, height: bounds.height)
        }
...
class RangeSlider: UIControl {
  ...
  @IBInspectable var lowerLayerSelected = Bool()
    ...
    @IBInspectable var thumbTintColor: UIColor = UIColor(rgb: 0xAB8FFE){
        didSet {
    ...
    @IBInspectable var thumbBorderColor: UIColor = UIColor(rgb: 0xAB8FFE) {
        didSet {
    ...
    fileprivate let lowerThumbLayer = RangeSliderThumbLayer(thumbPosition: .left)
    fileprivate let upperThumbLayer = RangeSliderThumbLayer(thumbPosition: .right)
    
    fileprivate var thumbWidth: CGFloat = 20
    let leftImage = UIImage(named: "ic_thumb_slider_left")
    let rightImage = UIImage(named: "ic_thumb_slider_right")
    ...
    func updateLayerFrames() {
        ...
        lowerThumbLayer.frame = CGRect(x: lowerThumbCenter - thumbWidth/2.0, y: -5.0, width: thumbWidth, height: 70)
        lowerThumbLayer.setNeedsDisplay()
        
        let upperThumbCenter = CGFloat(positionForValue(upperValue))
        upperThumbLayer.frame = CGRect(x: upperThumbCenter - thumbWidth/2.0, y: -5.0, width: thumbWidth, height: 70)
        upperThumbLayer.setNeedsDisplay()
